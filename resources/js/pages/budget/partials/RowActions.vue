...
const handleDelete = () => {
  router.delete(route("budget.destroy", row.id), {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  ...
  <FormDialog
    ...
    :header="{
      title: 'Edit Budget',
      description: 'You can update your budget detail by completing this form.',
    }"
    :default-values="row"
  />
  ...
  <ConfirmationDialog
    ...
    :description="`You will delete budget from category ${
      row.category.name
    } and month ${dateToMonth(row.month)}, this action can't be undone.`"
